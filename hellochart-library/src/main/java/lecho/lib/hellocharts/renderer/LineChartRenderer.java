...
public class LineChartRenderer extends AbstractChartRenderer {
    ...
    public LineChartRenderer(Context context, Chart chart, LineChartDataProvider dataProvider) {
        ...
        pointBorderPaint.setAntiAlias(true);
        pointBorderPaint.setStyle(Paint.Style.STROKE);
        pointBorderPaint.setStrokeWidth(2);
        pointBorderPaint.setColor(Color.parseColor("#2c3e50"));
    ...
    public void onChartDataChanged() {
        ...
        for (Line line : dataProvider.getLineChartData().getLines()) {
            if (line.getImageSize() > 0) {
                int size = ChartUtils.dp2px(scaledDensity, line.getImageSize());
                computator.setPaddingTopImage(size / 2);
                int pointRadius = ChartUtils.dp2px(density, line.getPointRadius());
                computator.setPaddingRadius(pointRadius);
            }
        }
    }
        ...
        for (Line line : data.getLines()) {
            if (checkIfShouldDrawPoints(line)) {
                ...
                if (margin > contentAreaMargin) {
                    //contentAreaMargin = margin; //Убирает отступы для графика с точками
                }
            }
        }
    ...
    private void drawPoints(Canvas canvas, Line line, int lineIndex, int mode) {
        ...
        for (int i = 0; i < line.getValues().size(); i++) {
         ...
         /*   String description;
            if (line.getDescriptions().size() != 0)
                description = line.getDescriptions().get(i);
            else
                description = "Hi";*/
            int pointRadius = ChartUtils.dp2px(density, line.getPointRadius());
            final float rawX = computator.computeRawX(pointValue.getX());
                ...
                if (MODE_DRAW == mode) {
                    ...
                    if (line.hasLabels()) {
                        ...
                        if (!line.hasDescription())
                            ...
                            //String description = line.getDescriptions().get(i);
                            Bitmap bitmap = line.getImages().get(i);
                            drawLabelDescription(canvas, line, pointValue, null, bitmap, rawX, rawY, pointRadius + labelOffset);
                        }
                    }
    ...
    private void drawPoint(Canvas canvas, Line line, PointValue pointValue, float rawX, float rawY,
        ...
        if (ValueShape.SQUARE.equals(line.getShape())) {
            ...
            canvas.drawCircle(rawX, rawY, pointRadius, pointBorderPaint);
    ...
    private void drawPointImage(Canvas canvas, Bitmap bitmap, float rawX, float rawY) {
        canvas.drawBitmap(bitmap, rawX - bitmap.getWidth() / 2, rawY - bitmap.getHeight() / 2, null);
    }
    ...
    /*    drawLabelTextAndBackground(canvas, labelBuffer, labelBuffer.length - numChars, numChars,
                line.getDarkenColor()); */
        drawLabelTextAndBackground(canvas, labelBuffer, labelBuffer.length - numChars, numChars,
                line.getLabelBackgroundColor(),
                line.getColor());
    }

    private void drawLabelDescription(Canvas canvas, Line line, PointValue pointValue, String description, Bitmap bitmap,
        ...
        labelBackgroundRect.set(left, top, right, bottom);
        //drawLabelTextAndBackground(canvas, labelBuffer, labelBuffer.length - numChars, numChars, description,
        //        line.getLabelBackgroundColor(),
        //        line.getColor());
        drawLabelImage(canvas, bitmap);
    }
...
/*        linePaint.setShader(line.getGradientToTransparent() ?
                new LinearGradient(0, canvas.getHeight() / 4, 0, canvas.getHeight()-canvas.getHeight()/8, Color.WHITE,
                        Color.WHITE & 0x00ffffff, Shader.TileMode.CLAMP) :
                        //Color.TRANSPARENT, Shader.TileMode.REPEAT) :
                null);*/

        linePaint.setShader(line.getGradientToTransparent() ?
                new LinearGradient(0, canvas.getHeight() / 2.0f, 0, contentRect.bottom, line.getAreaColor(),
                        //line.getColor() & 0x00ffffff, Shader.TileMode.CLAMP) :
                        //Color.TRANSPARENT, Shader.TileMode.REPEAT) :
