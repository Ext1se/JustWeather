...
public class ChartComputator {
    ...
    private int paddingTopImage = 0;
    private int paddingRadius = 0;
    /**
    ...
    public float computeRawY(float valueY) {
        float pixelOffset = (valueY - currentViewport.bottom) * (contentRectMinusAllMargins.height() /
                currentViewport.height()) - paddingTopImage;
        if (pixelOffset < paddingRadius) {
            pixelOffset = paddingRadius;
        }
    ...
    public void setPaddingTopImage(int paddingTopImage) {
        this.paddingTopImage = paddingTopImage;
    }
